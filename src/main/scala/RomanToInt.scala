...
def romanToInt(numeral: String): Int = {
  ...
  val (result, _) = (numeral.length - 1 to 0 by -1)
    .foldLeft((0, 0)) { case ((result, prev), i) =>
      val current = ValueTable(numeral(i))
      if (current < prev) {
